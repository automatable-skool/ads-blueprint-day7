...
### Spacing: tight for text, generous for layout

The scale splits at step 6. Steps 0–5 (2–16px) space text and the insides of controls and stay tight. Steps 6–13 (22–152px) are the layout steps — gaps between elements, card padding, the section rhythm — and are deliberately generous.

```
--space-6  22px    --space-10  64px
--space-7  28px    --space-11  84px     --pad-card      28px
--space-8  38px    --space-12 112px     --section-y-sm 112px
--space-9  48px    --space-13 152px     --section-y    152px
```

Sections breathe at 112px minimum, 152px where a page needs a beat. The template is sales collateral for a small business, not a dashboard: crowding is what makes a service site look cheap, and whitespace is the cheapest way to look established. If a page feels cramped, raise the layout step rather than trimming content.
...
5. **Footer** — `--accent-900` graphite, the only dark block on the page.

**Dark surfaces are graphite, never blue.** This applies to the site footer, the app sidebar and the dark theme alike. An all-blue dark surface puts blue cards on a blue page with blue text and reads as one muddy field — the `[data-theme="dark"]` block originally did exactly that (page `--blue-900`, cards `--blue-800`, body text `--blue-100`) and had to be rebuilt on a warm near-black ramp. On any dark surface, blue appears only where it appears on a white one: links, the primary button, the active nav item.
...
| Eyebrow labels | `--text-brand` blue, icon and text together — no pill, no background. On a white or tinted section the eyebrow is the section's one hit of blue. |
