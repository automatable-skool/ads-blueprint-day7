...
if (typeof document !== "undefined" && !document.getElementById("ms-field-css")) {
  const el = document.createElement("style"); el.id = "ms-field-css"; el.textContent = __css; document.head.appendChild(el);
}

export function Input({ label, hint, error, icon, size = "md", required = false, id, className = "", ...rest }) {
  const autoId = React.useId ? React.useId() : "ms-input";
  const inputId = id || autoId;
  return (
    <div className={["ms-field", className].filter(Boolean).join(" ")}>
      {label && <label className="ms-field__label" htmlFor={inputId}>{label}{required && <span className="ms-field__req">*</span>}</label>}
      <div className="ms-inputwrap">
        {icon && <span className="ms-inputwrap__icon"><Icon name={icon} size={18} /></span>}
        <input
          id={inputId}
          required={required}
