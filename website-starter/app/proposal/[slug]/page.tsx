// The proposal renderer - the ads edition. Light page, thin confidential bar,
// centered hero with the MISSED REVENUE number in the stat pills, then the
// seven sections. Dynamic: the data is fetched per request (Supabase row or the
// local registry), so the slug list is never frozen at build time.
//
// ⛔ ZERO ACCESS - nothing rendered here is a reading of the prospect's Google
// Ads account. The hero number is what the market is worth, not what they waste.
//
// noindex is mandatory in three places: metadata robots here, exclusion from
// app/sitemap.ts, and Disallow /proposal/ in app/robots.ts.
...
export default async function ProposalPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const stats: { label: string; value: string }[] = [
    ...
    { label: "Going elsewhere", value: `${data.missedRevenue.startingMonthly}/mo` },
    { label: "Fixed in", value: "30 days" },
      ...
      <section style={{ position: "relative", overflow: "hidden", background: "#f9f8f6", borderBottom: "1px solid #eceae6", padding: "clamp(110px, 14vw, 150px) 0 clamp(56px, 8vw, 84px)" }}>
        ...
        <div className="pp-inner" style={{ position: "relative", textAlign: "center", maxWidth: 880 }}>
          ...
          <h1 style={{ fontSize: "clamp(40px, 6.5vw, 68px)", fontWeight: 900, lineHeight: 1.05, letterSpacing: "-0.025em", margin: "26px 0 22px", color: d.text }}>
            There&rsquo;s <span className="pp-gradient">{data.missedRevenue.startingMonthly} a month</span> of work going to someone else
          </h1>
          ...
          {/* ⛔ ZERO ACCESS. This line used to read "{spendMonthly} a month in ad
              spend · read {windowLabel}" - both of which only exist inside the
              prospect's Google Ads account, which /proposal never opens. It now
              names the PUBLIC sources instead, which is also the thing that makes
              a stranger trust the numbers above it. Changed 1 September 2026. */}
          <p style={{ color: d.faint, fontSize: 13, margin: "26px 0 0" }}>
            {data.sources.label} &middot; {data.sources.readOn}
          </p>
          <p style={{ color: d.faint, fontSize: 13, margin: "6px 0 0" }}>
