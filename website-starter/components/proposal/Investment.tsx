// 05 - Investment. ONE recommended option with the accent border, and at most
// one smaller fallback beside it - never three equal choices, and never a
// discount line, because a discount says the first number was padded.
// Above the cards, the one line that sells it: the fee sits UNDER the monthly
// number from section 02.
...
export function Investment({ data }: { data: ProposalData }) {
  return (
    <Section
      ...
      kicker="The fix"
      title="Investment"
      lead="Fixed prices. Your ad spend stays yours and goes to Google directly - this is the fee for running it properly."
    >
      <div style={{ marginBottom: 18 }}>
        <Alert tone="caution">{data.investment.wasteReminder}</Alert>
      </div>
      <div className={data.investment.options.length > 2 ? "pp-3col" : "pp-2up"}>
