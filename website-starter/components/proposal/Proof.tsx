          ...
          <div key={p.name} className="pp-card" style={{ padding: 28, display: "flex", flexDirection: "column" }}>
            ...
            {p.videoUrl ? (
              <a href={p.videoUrl} target="_blank" rel="noopener noreferrer" style={{ display: "block", position: "relative", marginTop: "auto", paddingTop: 18, borderRadius: 12, overflow: "hidden" }}>
                ...
                <span style={{ position: "absolute", left: 12, bottom: 10, color: "#fff", fontSize: 13, fontWeight: 700, textShadow: "0 1px 6px rgba(0,0,0,.6)" }}>
                  Watch them say it
                </span>
              </a>
            ) : null}
            {p.disclaimer ? (
              <p style={{ color: d.faint, fontSize: 11.5, lineHeight: 1.5, margin: "10px 0 0" }}>{p.disclaimer}</p>
            ) : null}
          </div>
        ))}
      </div>
